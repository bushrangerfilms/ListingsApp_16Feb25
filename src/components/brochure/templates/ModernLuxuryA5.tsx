...
import {
  ModernCoverPageContent,
  ModernAccommodationPageContent,
  ModernFeaturesPageContent,
  ModernBackCoverPageContent,
} from './ModernLuxuryTemplate';
...
export function ModernLuxuryA5Reader({ content, branding }: A5Props) {
...
export function ModernLuxuryA5PrintReady({ content, branding }: A5Props) {
  const dims = getLayoutDimensions('a5');
  const typeOverrides = getTypeOverrides('a5');
  const ctx = buildPageRenderContext(content, branding, dims, typeOverrides);
